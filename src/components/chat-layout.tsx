...
export default function ChatLayout() {
  ...
  // Update active conversation ID when search params change
  useEffect(() => {
    ...
    <div className="flex h-full w-full border-gray-200 border-t">
            ...
            {chats.length === 0 ? (
              ...
              chats.map((chat: { id: string; partner_avatar?: string; partner_name?: string; post_title: string }) => (
                <div
                  key={chat.id}
                  className={cn(
                    "flex items-center px-3 py-3 rounded-lg cursor-pointer hover:bg-gray-100",
      ...
      <div className="w-2/3 h-full">
        {activeConversationId ? (
          <ChatComponent conversation_id={activeConversationId} />
        ) : (
